...
describe('GithubClient.getContentTree', () => {
  ...
  it('lanza error cuando repositoryName no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getContentTree('', 'main')).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
    await expect(client.getContentTree(null as any, 'main')).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
  })
  ...
  it('lanza error cuando nameBranch no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getContentTree('test-repo', '')).rejects.toThrow(
      'Name branch is required and must be a text string'
    )
    await expect(
      client.getContentTree('test-repo', null as any)
    ).rejects.toThrow('Name branch is required and must be a text string')
  })
  ...
  it('lanza error cuando la respuesta HTTP no es exitosa', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getContentTree('test-repo', 'main')).rejects.toThrow(
      'Github API Error: 404 - Not Found. Branch not found'
    )
  })
  ...
  it('lanza error cuando fetch falla', async () => {
    ...
    await client.initialize()
    await expect(client.getContentTree('test-repo', 'main')).rejects.toThrow(
      'Error getting workflows: Network error'
    )
  })
