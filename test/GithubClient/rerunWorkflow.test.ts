...
describe('GithubClient.rerunWorkflow', () => {
  ...
  it('lanza error cuando repositoryName no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.rerunWorkflow('', 123)).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
    await expect(client.rerunWorkflow(null as any, 123)).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
  })
  ...
  it('lanza error cuando runId no es number', async () => {
    ...
    await client.initialize()
    await expect(
      client.rerunWorkflow('test-repo', '123' as any)
    ).rejects.toThrow('Run id is required and must be a number')
    await expect(
      client.rerunWorkflow('test-repo', null as any)
    ).rejects.toThrow('Run id is required and must be a number')
  })
  ...
  it('lanza error cuando la respuesta HTTP no es exitosa', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.rerunWorkflow('test-repo', 999)).rejects.toThrow(
      'GitHub API Error: 403 - Forbidden. Workflow run not found or insufficient permissions'
    )
  })
  ...
  it('lanza error cuando fetch falla', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.rerunWorkflow('test-repo', 123)).rejects.toThrow(
      'Error rerun workflow: Network error'
    )
  })
