...
describe('GithubClient.createBranch', () => {
  ...
  it('lanza error cuando repositoryName no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.createBranch('', 'branch', 'sha')).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
    await expect(
      client.createBranch(null as any, 'branch', 'sha')
  ...
  it('lanza error cuando newBranchName no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.createBranch('test-repo', '', 'sha')).rejects.toThrow(
      'Branch name is required and must be a text string'
    )
    await expect(
      client.createBranch('test-repo', null as any, 'sha')
    ).rejects.toThrow('Branch name is required and must be a text string')
  })
  ...
  it('lanza error cuando sha no es string', async () => {
    ...
    await client.initialize()
    await expect(
      client.createBranch('test-repo', 'branch', '')
    ).rejects.toThrow('SHA is required and must be a text string')
    await expect(
      client.createBranch('test-repo', 'branch', null as any)
    ).rejects.toThrow('SHA is required and must be a text string')
  })
  ...
  it('lanza error cuando la respuesta HTTP no es exitosa', async () => {
    ...
    await client.initialize()
    await expect(
      client.createBranch('test-repo', 'existing-branch', 'sha')
    ).rejects.toThrow(
      'Github API Error: 422 - Unprocessable Entity. Branch already exists'
    )
  })
  ...
  it('lanza error cuando fetch falla', async () => {
    ...
    await client.initialize()
    await expect(
      client.createBranch('test-repo', 'branch', 'sha')
    ).rejects.toThrow('Error create branch: Network error')
  })
