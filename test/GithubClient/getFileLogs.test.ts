...
describe('GithubClient.getFileLogs', () => {
  ...
  it('lanza error cuando repositoryName no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getFileLogs('', 123)).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
    await expect(client.getFileLogs(null as any, 123)).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
  })
  ...
  it('lanza error cuando id no es number', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getFileLogs('test-repo', '123' as any)).rejects.toThrow(
      'Repository id is required and must be a number'
    )
    await expect(client.getFileLogs('test-repo', null as any)).rejects.toThrow(
      'Repository id is required and must be a number'
    )
  })
