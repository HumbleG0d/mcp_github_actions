...
describe('GithubClient.getDataWorkflows', () => {
  ...
  it('lanza error cuando repositoryName no es string', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getDataWorkflows('')).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
    await expect(client.getDataWorkflows(null as any)).rejects.toThrow(
      'Repository name is required and must be a text string'
    )
  })
  ...
  it('lanza error cuando workflow_runs no existe en la respuesta', async () => {
    ...
    const client = new GithubClient()
    await client.initialize()
    await expect(client.getDataWorkflows('test-repo')).rejects.toThrow(
      'GitHub response does not contain valid workflow_runs'
    )
  })
