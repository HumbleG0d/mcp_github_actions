...
describe('GithubClient.getContentFiles', () => {
    ...
    it('lanza error cuando repositoryName no es string', async () => {
        ...
        const client = new GithubClient()
        await client.initialize()
        await expect(client.getContentFiles('', 'path')).rejects.toThrow('Repository name is required and must be a text string')
        await expect(client.getContentFiles(null as any, 'path')).rejects.toThrow('Repository name is required and must be a text string')
    })
    ...
    it('lanza error cuando path no es string', async () => {
        ...
        const client = new GithubClient()
        await client.initialize()
        await expect(client.getContentFiles('test-repo', 123 as any)).rejects.toThrow('Path is required and must be a text string')
    })
    ...
    it('lanza error cuando la respuesta HTTP no es exitosa', async () => {
        ...
        const client = new GithubClient()
        await client.initialize()
        await expect(client.getContentFiles('test-repo', 'src/index.js')).rejects.toThrow('Github API Error: 404 - Not Found. File not found')
    })
    ...
    it('lanza error cuando fetch falla', async () => {
        ...
        const client = new GithubClient()
        await client.initialize()
        await expect(client.getContentFiles('test-repo', 'src/index.js')).rejects.toThrow('Error getting bring content repo: Network error')
    })
