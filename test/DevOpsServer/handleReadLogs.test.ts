...
describe('DevOpsMCPServer - handleReadLogs', () => {
    ...
    beforeEach(() => {
        ...
        mockReadMultiplesFile = readMultiplesFile as jest.MockedFunction<typeof readMultiplesFile>;
    ...
    describe('Éxito', () => {
        ...
        it('debería manejar logs con caracteres especiales', async () => {
            // Arrange
            const mockLogContent = {
                'special-chars.log': 'Error: "Failed to parse JSON: {\'key\': \'value\'}"\nPath: C:\\Users\\Test\\file.txt',
                'unicode.log': 'Error: '
            };
            const params: ToolParams = {
                repositoryName: '',
    ...
    describe('Casos edge', () => {
        it('debería manejar nombres de directorio con caracteres especiales', async () => {
            ...
            // Assert
            expect(mockReadMultiplesFile).toHaveBeenCalledWith('/path/with spaces/and-special@chars#');
        ...
        it('debería manejar múltiples archivos de log con diferentes formatos', async () => {
            ...
            expect(result.content[0].text).toContain('INFO: Process completed successfully');
            ...
            expect(result.content[0].text).toContain('127.0.0.1 - - [01/Jan/2024:00:00:00 +0000]');
        });
    });
});
