...
describe('DevOpsMCPServer - handleRerunWorkflow', () => {
    ...
    describe('Éxito', () => {
        it('debería re-ejecutar un workflow cuando la operación es exitosa', async () => {
            ...
            expect(mockGithubClient.rerunWorkflow).toHaveBeenCalledWith('test-repo', 123);
        ...
        it('debería manejar diferentes estados de re-ejecución', async () => {
            // Arrange
            const statuses = ['queued', 'in_progress', 'completed', 'cancelled', 'failed'];
            const params: ToolParams = {
                repositoryName: 'test-repo',
                id: 123,
                dirName: '',
            ...
            for (const status of statuses) {
                ...
                // Assert
                expect(result.content[0].text).toContain(`Workflow rerun with status: ${status}`);
        ...
        it('debería manejar IDs de workflow muy grandes', async () => {
            ...
            // Assert
            expect(mockGithubClient.rerunWorkflow).toHaveBeenCalledWith('test-repo', largeId);
    ...
    describe('Casos edge', () => {
        it('debería manejar nombres de repositorio con caracteres especiales', async () => {
            ...
            // Assert
            expect(mockGithubClient.rerunWorkflow).toHaveBeenCalledWith('repo-with-special-chars-@#$%', 123);
        ...
        it('debería manejar IDs de workflow negativos', async () => {
            ...
            // Assert
            expect(mockGithubClient.rerunWorkflow).toHaveBeenCalledWith('test-repo', -1);
        ...
        it('debería manejar IDs de workflow cero', async () => {
            ...
            // Assert
            expect(mockGithubClient.rerunWorkflow).toHaveBeenCalledWith('test-repo', 0);
        ...
        it('debería manejar parámetros con valores undefined', async () => {
            ...
            // Assert
            expect(mockGithubClient.rerunWorkflow).toHaveBeenCalledWith('test-repo', undefined);
        ...
        it('debería manejar múltiples workflows en diferentes repositorios', async () => {
            ...
            for (let i = 0; i < repositories.length; i++) {
                ...
                // Assert
                expect(result.content[0].text).toContain(`Workflow ${workflowIds[i]} rerun in ${repositories[i]}`);
            }
        });
    });
});
