...
export class GithubClient implements GithubAPI {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to initialize GitHub client: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  async getDataWorkflows(repositoryName: string): Promise<GitHubWorkflowRun[]> {
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/actions/runs`,
        {
          method: 'GET',
          headers: this.buildHeaders('application/vnd.github.v3+json'),
        }
      ...
      const filteredData: GitHubWorkflowRun[] = data.workflow_runs.map(
        (run) => ({
          id: run.id,
          name: run.name,
  ...
  async getFileLogs(
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/actions/runs/${encodeURIComponent(id)}/logs`,
        {
          redirect: 'follow',
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Unknown error getting file logs',
      }
    }
  }
  ...
  async getContentTree(
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/git/trees/${encodeURIComponent(nameBranch)}?recursive=1`,
        {
          method: 'GET',
          headers: this.buildHeaders('application/vnd.github.v3+json'),
        }
  ...
  async updateFile(
    ...
    try {
      ...
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/contents/${encodeURIComponent(path)}`,
        {
          method: 'PUT',
          headers: this.buildHeaders('application/vnd.github.v3+json'),
          body: JSON.stringify({
  ...
  async createBranch(
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/git/refs`,
        {
          method: 'POST',
          headers: this.buildHeaders('application/vnd.github.v3+json'),
          body: JSON.stringify({
  ...
  async rerunWorkflow(
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/actions/runs/${encodeURIComponent(runId)}/rerun`,
        {
          method: 'POST',
          headers: this.buildHeaders('application/vnd.github.v3+json'),
        }
  ...
  async getStatusWorkflow(
    ...
    try {
      const response = await fetch(
        `https://api.github.com/repos/${this.user}/${encodeURIComponent(repositoryName)}/actions/runs/${encodeURIComponent(runId)}`,
        {
          method: 'GET',
          headers: this.buildHeaders('application/vnd.github.v3+json'),
        }
